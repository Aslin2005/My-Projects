...
fir_genvar #(.TAPS(100), .DATA_WIDTH(16), .COEFF_WIDTH(16), .ACC_WIDTH(32)) dut (
    .clk(clk),
    .reset(reset),
    .x_in(x_in),
    .y_out(y_out)
...
reg signed [15:0] test_data [0:999];
...
initial begin
    for (f = 0; f < 3; f = f + 1) begin
        case (f)
            0: begin
                ...
                file_name_out = "verilog_genvar_950Hz.txt";
            end
            1: begin
                file_name_in  = "signal_1100Hz.txt";
                file_name_out = "verilog_genvar_1100Hz.txt";
            end
            2: begin
                file_name_in  = "signal_2000Hz.txt";
                file_name_out = "verilog_genvar_2000Hz.txt";
            end
        endcase

        $display("Opening %s ...", file_name_in);
    ...
    $display("All Genvar Form simulations complete");
    $finish;
end
